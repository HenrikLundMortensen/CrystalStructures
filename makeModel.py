...
def loaddata(Ndata, Natoms, Ndim):
    ...
    while Ndata_added < Ndata:
        ...
        Xadd = np.loadtxt(fname='relaxor_sandbox/data50/positions' + str(k) + '.dat', delimiter='\t')
        Eadd = np.loadtxt(fname='relaxor_sandbox/data50/energies' + str(k) + '.dat', delimiter='\t')
...
def loaddatafile(Ndata, Natoms, Ndim, i_file):
    ...
    # Load data from datafile number k
    X = np.loadtxt(fname='relaxor_sandbox/data50/positions' + str(i_file) + '.dat', delimiter='\t')
    E = np.loadtxt(fname='relaxor_sandbox/data50/energies' + str(i_file) + '.dat', delimiter='\t')
...
if __name__ == "__main__":
    ...
    np.savetxt('features' + str(0) + '.dat', Xf, delimiter='\t')
